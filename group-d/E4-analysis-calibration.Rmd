...
m_calib = broom::tidy(lm(bias_effect ~ mean_percent_in_roi, data = eyetracking.effects.by.subj))
#m_calib 
```

```{r}
m_calib_b = m_calib %>% filter(term ==  'mean_percent_in_roi') %>% pull(estimate ) %>% round(digits = 3)
m_calib_se = m_calib %>% filter(term ==  'mean_percent_in_roi') %>% pull(std.error ) %>% round(digits = 3)
m_calib_p = m_calib %>% filter(term ==  'mean_percent_in_roi') %>% pull(p.value ) %>% round(digits = 3)
```
...
ggplot(eyetracking.effects.by.subj, aes(x = mean_percent_in_roi, y = bias_effect))+
  ...
  geom_smooth(method = "lm")
```
Participants' calibration quality, measured as the mean percentage of fixations that landed within XX pixels of the calibration point, varied substantially (`r range(calib.by.subj$mean_percent_in_roi) |> round(digits = 2)`). 
The quality of a participant's calibration significantly predicted the participant's effect size. 
The difference in target animal fixation proportions between modifier and instrument conditions was higher for participants with better calibration ($b=$`r m_calib_b`,$SE=$`r m_calib_se`, $p=$`r m_calib_p`). 
