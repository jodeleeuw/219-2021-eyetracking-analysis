...
task.data <- all.data %>%
  ...
  dplyr::select(user_id, trial_index, rt, images, webgazer_data, mouse_events, compatibility, audio, target_instrument, target_animal, webgazer_targets)
...
first.move <- mousetracking.data %>%
  select(user_id, trialID, object, type, compatibility, target_instrument, target_animal, images) %>%
  group_by(user_id, trialID) %>%
  filter(type=="enter") %>%
  slice(1) %>%
  rowwise() %>%
...
first.move.subject.summary <- first.move %>%
  group_by(user_id, compatibility) %>%
...
eyetracking.figure.2.data <- eyetracking.data.with.time.windows %>%
  ...
  group_by(user_id, compatibility, t.window) %>%
