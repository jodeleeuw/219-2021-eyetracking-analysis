...
Altmann & Kamide tested a second model, aligning the predictive time window with the offset of the verb rather than the onset of the noun as above. When we did the same, we again saw that the critical interaction is not significant nor in the expected direction (_b_ = `r jh.E1_model2_obj_verb.min %>% pull(estimate) %>% round(digits = 2)`, _SE_ =  `r jh.E1_model2_obj_verb.min %>% pull(std.error) %>% round(digits = 2)`, _p_=`r jh.E1_model2_obj_verb.min %>% pull(p.value) %>% round(digits = 2)`).
...
```
```{r E1b-calib-effect, fig.cap='Calibration scores plotted against the verb effect, operationalized as the latency of the first target fixation in the non-restricting verb condition minus that in the restricting verb condition.'}
...
Again, we excluded participants with validation accuracy under `r validation.threshold.aggro.jh`%, which eliminated `r length(bad.subjects.validation.aggro.jh)` participants. The following analyses included `r nlevels(droplevels(critical.trials.filtered.aggro.jh$subject_id))` participants. 
...
```{r E1b-spaghetti-fig, fig.cap = "Cumulative probability of fixating distractor and target objects across conditions over time, with 0 ms aligned to the verb onset time. The vertical line marks the mean noun onset time across trials and conditions."} 
