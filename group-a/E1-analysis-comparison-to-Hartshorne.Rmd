...
Across the three condition effects of interest, calibration quality was not significantly correlated (pre-noun: Pearson's _r_ = `r jh.m1_diffdiff_cor_r`, _p_ = `r jh.m1_diffdiff_cor_p`; pre-verb-offset: Pearson's _r_ = `r jh.m2_diffdiff_cor_r`, _p_ = `r jh.m2_diffdiff_cor_p`; first fixation: Pearson's _r_ = `r jh.m3_diff_cor_r`, _p_ = `r jh.m3_diff_cor_p`. 

In the remote sample, we saw that calibration quality was correlated with target advantage in the pre-noun window in the restricting condition alone. This was not the case in the in-lab sample (Pearson's _r_ = `r jh.m1_diff_cor_r`, _p_ = `r jh.m1_diff_cor_p`).
...
```
#### Aggressive Exclusion
The second set of analyses used more aggressive exclusion criteria. First, we eliminated participants with `r prop.samples.threshold.aggro.jh*100` percent of fixations or fewer landing in any ROIs. This resulted in the elimination of `r length(bad.subjects.prop.samples.aggro.jh)` participants. Second, we excluded participants with validation accuracy under `r validation.threshold.aggro.jh` percent, which eliminated an additional `r n.bad.subjects.aggro.jh - length(bad.subjects.prop.samples.aggro.jh)` participants. The following analyses included `r nlevels(droplevels(critical.trials.filtered.aggro.jh$subject_id))` participants. 
