...
jh.validation.percent <- jh.validation.accuracy.data %>%
  group_by(subject_id) %>%
  #slice_tail() %>%
  select(subject_id, percent_in_roi, trial_index, phase) %>%
...
jh.critical.trials.filtered.min <- jh.critical.trials #The name is a lie :)
...
```{r make jh spaghetti plot} 

jh_E1_spag_fig <- ggplot(jh.cumulative.fixation.data.summary.min, aes(x=t.window, y=cumulative.fixation.m, fill=object_class, shape=verb_type, group=interaction(verb_type, object_class)))+
  geom_line()+
...
```
#### Pre-noun fixations 
We ran the analysis exactly as before. Unlike with the remote sample, this time there was a significant main effect of object type such that participants were more likely to be looking at the target than the distracter object during this time window (_b_ = `r jh.E1_model1_obj.min %>% pull(estimate) %>% round(digits = 2)`, _SE_ =  `r jh.E1_model1_obj.min %>% pull(std.error) %>% round(digits = 2)`, _p_=`r jh.E1_model1_obj.min %>% pull(p.value) %>% round(digits = 2)`). However, the critical interaction was again not statistically significant and indeed in the wrong direction (_b_ = `r jh.E1_model1_obj_verb.min %>% pull(estimate) %>% round(digits = 2)`, _SE_ =  `r jh.E1_model1_obj_verb.min %>% pull(std.error) %>% round(digits = 2)`, _p_=`r jh.E1_model1_obj_verb.min %>% pull(p.value) %>% round(digits = 2)`). 
...
When we repeated this analysis, the critical interaction was again not significant, nor was it even in the expected direction (_b_ = `r jh.E1_model2_obj_verb.min %>% pull(estimate) %>% round(digits = 2)`, _SE_ =  `r jh.E1_model2_obj_verb.min %>% pull(std.error) %>% round(digits = 2)`, _p_=`r jh.E1_model2_obj_verb.min %>% pull(p.value) %>% round(digits = 2)`).
...
```{r E1-latency-table-min-JH, echo=FALSE}
...
jh.E1_model3_dur.min = jh.E1_model3_tab.min %>% filter(term == "dur_verb_ms_c")
jh.E1_model3_intxn.min = jh.E1_model3_tab.min %>% filter(term == "verb_typerestricting:dur_verb_ms_c")
```
#### First target fixations after verb
On average, participants looked to the target `r jh.our_table1_min$Eat[4]` ms after the noun onset in the restrictive condition and `r jh.our_table1_min$Move[4]` ms after the noun onset in the non-restrictive condition. As in the remote sample, the latencies are overall slower than in results published by Altmann & Kamide (1999) and James et al. (2023). Unlike in the remote sample, however, the difference is in the unexpected direction, such that participants looked to the target faster in the non-restrictive condition. In any case, no effects were significant; results revealed that the paradoxical advantage in the non-restrictive condition was not statistically significant (_b_ = `r jh.E1_model3_verb.min %>% pull(estimate) %>% round(digits = 2)`, _SE_ =  `r jh.E1_model3_verb.min %>% pull(std.error) %>% round(digits = 2)`, _p_=`r jh.E1_model3_verb.min %>% pull(p.value) %>% round(digits = 2)`). Effects of verb duration and its interaction with condition were also not statistically significant (duration: _b_ = `r jh.E1_model3_dur.min %>% pull(estimate) %>% round(digits = 2)`, _SE_ =  `r jh.E1_model3_dur.min %>% pull(std.error) %>% round(digits = 2)`, _p_=`r jh.E1_model3_dur.min %>% pull(p.value) %>% round(digits = 2)`; interaction: _b_ = `r jh.E1_model3_intxn.min %>% pull(estimate) %>% round(digits = 2)`, _SE_ =  `r jh.E1_model3_intxn.min %>% pull(std.error) %>% round(digits = 2)`, _p_=`r jh.E1_model3_intxn.min %>% pull(p.value) %>% round(digits = 2)` )
...
```{r E1b-calib-effect, fig.cap='Calibration scores in Exp. 1b, plotted against the verb effect. This was operationalized as the latency of the first target fixation in the non-restricting verb condition minus that in the restricting verb condition.', out.width="50%", fig.align="center"}
#I picked this one because it's the same as in E1a
ggplot(jh.cali.analy.3, aes(x=M, y=diff))+
  geom_point()+
  geom_smooth(method="lm", formula = y ~ x)+
  labs(x="Calibration Score", y="Verb Effect (ms)")+
  theme_bw()+
  theme(panel.grid.minor = element_blank()) +
  theme(panel.grid.minor = element_blank(),
    axis.title = element_text(size=14),      
...
As in Exp. 1a, calibration quality was not significantly correlated with any of the three effects (pre-noun: Pearson's _r_ = `r jh.m1_diffdiff_cor_r`, _p_ = `r jh.m1_diffdiff_cor_p`; pre-verb-offset: Pearson's _r_ = `r jh.m2_diffdiff_cor_r`, _p_ = `r jh.m2_diffdiff_cor_p`; first fixation: Pearson's _r_ = `r jh.m3_diff_cor_r`, _p_ = `r jh.m3_diff_cor_p`. Figure\ \@ref(fig:E1-calib-effect) plots the relation between calibration scores and Effect 3.
...
```{r}
#AJ: not actually using this
prop.samples.threshold.aggro.jh <- 0.2

bad.subjects.prop.samples.aggro.jh <- jh.prop.in.roi.by.subject %>%
  filter(prop.of.samples.in.roi <= prop.samples.threshold.aggro.jh) %>%
  pull(subject_id)
```
...
```{r}
validation.threshold.aggro.jh <- 50


bad.subjects.validation.aggro.jh <- jh.validation.percent %>%
  filter(M <= validation.threshold.aggro.jh) %>%
  pull(subject_id) %>%
  as.character()
```
...
```{r}
bad.subjects.aggro.jh <- unique(c(bad.subjects.prop.samples.aggro.jh, bad.subjects.validation.aggro.jh))

critical.trials.filtered.aggro.jh <- jh.critical.trials %>%
  #filter(!subject_id %in% bad.subjects.aggro.jh)
  filter(!subject_id %in% bad.subjects.validation.aggro.jh)

n.bad.subjects.aggro.jh <- length(bad.subjects.aggro.jh)
n.bad.subjects.validation.aggro.jh <- length(bad.subjects.validation.aggro.jh)
```
...
```{r}
cumulative.fixation.data.aggro.jh <- critical.trials.filtered.aggro.jh %>%
  mutate(t.relative.to.verb.onset = t - (verb_onset*1000)) %>%
  filter(t.relative.to.verb.onset >= 0 & t.relative.to.verb.onset <2000) %>%
  mutate(t.w = floor(t.relative.to.verb.onset/50)*50)
```
...
if(file.exists("data/generated/cumulative_fixation_data_aggro.jh.csv")){
  cumulative.fixation.windows.data.aggro.jh <- read_csv("data/generated/cumulative_fixation_data_aggro.jh.csv")
} else {

  cumulative.fixation.windows.data.aggro.jh <- cumulative.fixation.data.aggro.jh %>%
    group_by(subject_id, verb_type, image_name, object_type, object_class) %>%
    summarize(cumulative.fixation.calculation(cur_data()))
  
  write_csv(cumulative.fixation.windows.data.aggro.jh, file ="data/generated/cumulative_fixation_data_aggro.jh.csv")
}
```
...
```{r}
cumulative.fixation.windows.grouped.data.aggro.jh <- cumulative.fixation.windows.data.aggro.jh %>%
  filter(!subject_id %in% bad.subjects.validation.aggro.jh) %>%
  group_by(subject_id, verb_type, image_name, object_class, t.window) %>%
  summarize(cumulative.fixation.p = mean(has.fixated)) 
```
...
```{r}
cumulative.fixation.windows.grouped.average.data.aggro.jh <- cumulative.fixation.windows.grouped.data.aggro.jh %>%
  group_by(subject_id, verb_type, object_class, t.window) %>%
  summarize(cumulative.fixation.p = mean(cumulative.fixation.p))
```
...
```{r}
cumulative.fixation.data.summary.aggro.jh <- cumulative.fixation.windows.grouped.average.data.aggro.jh %>%
  filter(object_class %in% c('target.object', 'distractor')) %>%
  group_by(t.window, verb_type, object_class) %>%
  summarize(cumulative.fixation.m = mean(cumulative.fixation.p))
```
...
E1_spag_fig_aggro.jh <- ggplot(cumulative.fixation.data.summary.aggro.jh, aes(x=t.window, y=cumulative.fixation.m, fill=object_class, shape=verb_type, group=interaction(verb_type, object_class)))+
...
```{r}
noun.onset.times.aggro.jh <- critical.trials.filtered.aggro.jh %>%
  group_by(image_name, verb_type) %>%
  slice(1) %>%
  ungroup() %>%
  select(image_name, verb_type, delay_bw_onset_of_verb_and_onset_of_target_noun) %>%
...
trial.level.cumulative.fixation.data.aggro.jh <- cumulative.fixation.windows.grouped.data.aggro.jh %>%
  left_join(noun.onset.times.aggro.jh, by=c("image_name", "verb_type"))
```

<!--Then, grab the last time window before the noun onset for each trial. This is the critical data for this test.-->

```{r}
pre.noun.first.looks.data.aggro.jh <- trial.level.cumulative.fixation.data.aggro.jh %>%
  group_by(subject_id, image_name, verb_type) %>%
  filter(t.window < relative.noun.onset) %>%
  filter(t.window == max(t.window)) %>%
  filter(object_class %in% c("target.object", "distractor"))
...
```{r}
pre.noun.first.looks.data.aggro.jh$verb_type <- as.factor(pre.noun.first.looks.data.aggro.jh$verb_type)
pre.noun.first.looks.data.aggro.jh$object_class <- as.factor(pre.noun.first.looks.data.aggro.jh$object_class)
```
<!--Fit the model, with no covariance between random effects because the model cannot converge with full covariance matrix.-->

```{r}
E1_model1.aggro.jh <- afex::lmer_alt(cumulative.fixation.p ~ object_class * verb_type + (object_class * verb_type || image_name) + (object_class * verb_type || subject_id) , data=pre.noun.first.looks.data.aggro.jh)
#summary(E1_model1.aggro.jh)

E1_model1_tab.aggro.jh = broom.mixed::tidy(E1_model1.aggro.jh) 
E1_model1_obj.aggro.jh = E1_model1_tab.aggro.jh %>% filter(term == "object_classtarget.object")
E1_model1_verb.aggro.jh = E1_model1_tab.aggro.jh %>% filter(term == "verb_typerestricting")
E1_model1_obj_verb.aggro.jh = E1_model1_tab.aggro.jh %>% filter(term == "object_classtarget.object:verb_typerestricting")
```
...
```{r}
verb.offset.times.aggro.jh <- critical.trials.filtered.aggro.jh %>%
  group_by(image_name, verb_type) %>%
  slice(1) %>%
  ungroup() %>%
  select(image_name, verb_type, duration_of_verb) %>%
...
trial.level.cumulative.fixation.data.verb.offset.aggro.jh <- cumulative.fixation.windows.grouped.data.aggro.jh %>%
  left_join(verb.offset.times.aggro.jh, by=c("image_name", "verb_type"))
```

```{r}
pre.verb.offset.first.looks.data.aggro.jh <- trial.level.cumulative.fixation.data.verb.offset.aggro.jh %>%
  group_by(subject_id, image_name, verb_type) %>%
  filter(t.window < relative.verb.offset) %>%
  filter(t.window == max(t.window)) %>%
  filter(object_class %in% c("target.object", "distractor"))
...
E1_model2.aggro.jh <- afex::lmer_alt(cumulative.fixation.p ~ object_class * verb_type + (object_class * verb_type || image_name) + (object_class * verb_type || subject_id) , data=pre.verb.offset.first.looks.data.aggro.jh)
#summary(E1_model2.aggro.jh)

E1_model2_tab.aggro.jh = broom.mixed::tidy(E1_model2.aggro.jh) 
E1_model2_obj.aggro.jh = E1_model2_tab.aggro.jh %>% filter(term == "object_classtarget.object")
E1_model2_verb.aggro.jh = E1_model2_tab.aggro.jh %>% filter(term == "verb_typerestricting")
E1_model2_obj_verb.aggro.jh = E1_model2_tab.aggro.jh %>% filter(term == "object_classtarget.object:verb_typerestricting")
```

<!--Relative to noun onset, when is the first (post-verb) fixation on the target?-->

```{r}
first.fixation.after.verb.onset.aggro.jh <- critical.trials.filtered.aggro.jh %>%
  group_by(subject_id, image_name) %>%
  filter(t >= verb_onset*1000) %>%
  filter(object_type == "target.object") %>%
  filter(is.fixating == TRUE) %>%
...
```{r}
first.fixation.after.verb.onset.analysis.data.aggro.jh <- first.fixation.after.verb.onset.aggro.jh %>%
  ungroup() %>%
  select(subject_id, image_name, verb_type, duration_of_verb, noun_relative.t) %>%
  mutate(dur_verb_ms_c = scale(duration_of_verb, scale = FALSE)*1000) #mean-center verb duration
```
...
```{r}
E1_model3.aggro.jh <- lmer_alt(noun_relative.t ~ verb_type*dur_verb_ms_c + (verb_type || subject_id) + (verb_type || image_name), data=first.fixation.after.verb.onset.analysis.data.aggro.jh)

#summary(E1_model3.aggro.jh)

E1_model3_tab.aggro.jh = broom.mixed::tidy(E1_model3.aggro.jh) 
E1_model3_verb.aggro.jh = E1_model3_tab.aggro.jh %>% filter(term == "verb_typerestricting")

```
#### Re-analysis After Exclusions
Again, we re-analyzed the data focusing on participants with validation accuracy of at least `r validation.threshold.aggro.jh`% (N = `r nlevels(droplevels(critical.trials.filtered.aggro.jh$subject_id))`). Across all three models, results were in line with analyses using the minimal exclusion criteria; none of the critical effects were statistically significant, nor were they in the expected direction (Pre-noun-onset window, verb x object interaction: _b_ = `r E1_model1_obj_verb.aggro.jh %>% pull(estimate) %>% round(digits = 2)`, _SE_ =  `r E1_model1_obj_verb.aggro.jh %>% pull(std.error) %>% round(digits = 2)`, _p_=`r E1_model1_obj_verb.aggro.jh %>% pull(p.value) %>% round(digits = 2)`; Pre-verb-offset window, verb x object interaction: _b_ = `r E1_model2_obj_verb.aggro.jh %>% pull(estimate) %>% round(digits = 2)`, _SE_ =  `r E1_model2_obj_verb.aggro.jh %>% pull(std.error) %>% round(digits = 2)`, _p_=`r E1_model2_obj_verb.aggro.jh %>% pull(p.value) %>% round(digits = 2)`; Verb effect on first target fixation: _b_ = `r E1_model3_verb.aggro.jh %>% pull(estimate) %>% round(digits = 2)`, _SE_ =  `r E1_model3_verb.aggro.jh %>% pull(std.error) %>% round(digits = 2)`, _p_=`r E1_model3_verb.aggro.jh %>% pull(p.value) %>% round(digits = 2)`).


```{r E1b-spaghetti-fig, fig.cap = "Cumulative probability of fixating distractor and target objects across conditions over time, with 0 ms aligned to the verb onset time. Results from our web-based study (Panel A; Experiment 1a) are contrasted with the current results (Panel B). The vertical line marks the mean noun onset time across trials and conditions.", out.width="75%", fig.align="center"} 

E1_spag_fig_remote <- E1_spag_fig_remote +    theme(
...
E1_panel_fig <- E1_spag_fig_remote / jh_E1_spag_fig
E1_panel_fig + plot_annotation(tag_levels = 'A') + plot_layout(guides = 'collect') +
    theme(
    panel.grid.minor = element_blank(),
    axis.title = element_text(size=14),      # Axis titles
    axis.text = element_text(size=14),       # Axis labels
