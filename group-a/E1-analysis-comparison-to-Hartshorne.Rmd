...
---
<!-- # Setup -->
```{r E1-setup, include = FALSE, echo = FALSE}
library(dplyr)
library(tidyr)
library(jsonlite)
library(ggplot2)
...
```{r Merge stim info, message=FALSE, warning=FALSE, include=FALSE}
...
```{r Word durations, message=FALSE, warning=FALSE, include=FALSE}
...
```{r prop-in-ROI, message=FALSE, warning=FALSE, include=FALSE}
...
```
<!-- This should maybe go at the beginning of analysis section so that I just say it once? -->
__Regions of Interest__. ROIs were defined by creating boxes around each object in the scene. The size of each box was determined by taking the height and width of the given object and adding 20 pixels of padding. Each scene contained an agent region, a target region, and three or four distractor regions. 
...
```{r set up 50 ms bins, message=FALSE, warning=FALSE}
...
```{r slowly build cumu fix data, message=FALSE, warning=FALSE}
...
if(file.exists("data/data-in-lab-hartshorne/generated/jh.cumulative_fixation_data_min.csv")){
  jh.cumulative.fixation.windows.data.min <- read_csv("data/generated/jh.cumulative_fixation_data_min.csv")
} else {
  ...
  write_csv(jh.cumulative.fixation.windows.data.min, file="data/generated/jh.cumulative_fixation_data_min.csv")
}
```
...
```{r run model 1, message=FALSE, warning=FALSE}
...
```{r run model 2, message=FALSE, warning=FALSE}
...
```
__Pre-verb-offset fixations.__ Altmann & Kamide tested a second model, aligning the predictive time window with the offset of the verb rather than the onset of the noun as above. When we do the same, we again see that the critical interaction is not significant but numerically in the expected direction (_b_ = `r jh.E1_model2_obj_verb.min %>% pull(estimate) %>% round(digits = 2)`, _SE_ =  `r jh.E1_model2_obj_verb.min %>% pull(std.error) %>% round(digits = 2)`, _p_=`r jh.E1_model2_obj_verb.min %>% pull(p.value) %>% round(digits = 2)`).
...
```{r run model 3, message=FALSE, warning=FALSE}
