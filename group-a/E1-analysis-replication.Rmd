...
Altmann and Kamide tested a second model, aligning the predictive time window with the offset of the verb rather than the onset of the noun as above. When we do the same, we again see that the critical interaction is not significant but numerically in the expected direction (_b_ = `r E1_model2_obj_verb.min %>% pull(estimate) %>% round(digits = 2)`, _SE_ =  `r E1_model2_obj_verb.min %>% pull(std.error) %>% round(digits = 2)`, _p_=`r E1_model2_obj_verb.min %>% pull(p.value) %>% round(digits = 2)`).
...
Finally, we address whether participants look to the target faster in the restrictive vs. the non-restrictive condition, starting after the onset of the verb. [TO-DO: On average, participants looked to the target `r E1_latency_table_min$Eat.x[2]` ms after the noun onset in the restrictive condition (compared to ) and `r E1_latency_table_min$Eat.x[2]` ms after the noun onset in the non-restrictive condition (compared to )]. Thus, first fixations were not only delayed relative to those in the previous studies compared here, but also showed a smaller difference between conditions. 
