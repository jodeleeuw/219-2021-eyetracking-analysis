...
plot.AR<-ggplot(rescore, aes(x=time.window, y=prop.in.target, color=task))+
  geom_point(aes(color = task))+ 
  scale_fill_manual(name = "Task",values=c("red", "turquoise4"), labels = c("Attractiveness", "Roundness")) +
  geom_line(data=fit.A.dat, aes(x=x, y=y), color="red", lwd=1.25, linetype = "dashed") + 
  geom_line(data=fit.R.dat, aes(x=x, y=y), color="turquoise4", lwd=1.25, linetype = "solid") + 
  scale_x_reverse() + theme_classic() + 
  labs(x="Time until decision (ms)", y="Proportion of gaze in target", color="Task", fill="Task") + ylim(.4, 1) +     theme(
...
(ref:groupEMainrev-caption-separate) Revised results for Exp. 5 after removing low-calibration accuracy participants. *Left*: Eyegaze during attractiveness judgments, along with the best-fitting sigmoid. *Right*: Eyegze during roundness judgments, along with best-fitting sigmoid (best-fitting sigmoid for attractiveness is re-plotted with a dashed line for comparison).

(ref:groupEMain-caption) Revised results for Exp. 5 after removing low-calibration accuracy participants. The proportion of fixations to the target in the two seconds prior to the judgment are shown for the attractiveness (red) and roundness (blue) conditions along with their best-fitting sigmoids.

```{r groupEMainrev, fig.show="hold", out.width="45%", fig.align="center", echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "(ref:groupEMainrev-caption)"}
#plot.A
#plot.R
plot.AR
```
