...
(ref:E5cal-caption) Histogram of calibration success in Exp. 5. Where participants required more than one calibration (N=8), only the final calibration was considered.

```{r E5cal, fig.show="hold", out.width="45%", echo=FALSE, message=FALSE, warning=FALSE, fig.cap="(ref:E5cal-caption)"}
...
To determine whether calibration accuracy influenced our key effects, we calculated the percentage of samples during the task in which the participant was fixating the face they ultimately chose. There was a significant correlation for both the attractiveness judgments (r = `r round(scorescorrs.A$estimate, 2)` [`r round(scorescorrs.A$conf.int, 2)[1]`, `r round(scorescorrs.A$conf.int, 2)[2]`], p = `r round(scorescorrs.A$p.value, 2)`) and the roundness judgments (r = `r round(scorescorrs.R$estimate, 2)` [`r round(scorescorrs.R$conf.int, 2)[1]`, `r round(scorescorrs.R$conf.int, 2)[2]`], p = `r round(scorescorrs.R$p.value, 2)`). Inspection of Fig. \@ref(fig:E5calcorr) reveals that this correlation is due to a handful of particpiants with calibration values below 50%.

(ref:E5calcorr-caption) Correlation between calibration accuracy (x-axis) and percentage of samples fixating target (y-axis) in Exp. 5.
...
ggplot(scores, aes(x=mean_percent_in_roi, y=prop.in.target.subject, group=task, color=task))+
  ...
  labs(x="calibration accuracy", y="looking at chosen face") +
  theme_classic() +
  theme(panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank(),
        panel.grid.minor.y = element_blank(),
