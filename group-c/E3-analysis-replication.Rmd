...
The mean looking score was `r mean(t.test.data1$M.looking.score)` (*SD* = `r sd(t.test.data1$M.looking.score)`). This significantly greater than 0.5, *t*(`r t.test.day1$parameter`) = `r t.test.day1$statistic`, *p* = `r t.test.day1$p.value`, indicating that participants did show a preference for looking at the novel objects.
...
```{r plot-cumulative-looking-score, echo=FALSE, fig.cap="Cumulative looking score over the 5 second exposure during part 2 of day 1. Error bars represent +/- 1 SEM."}
...
```{r Plot Looking Score Correlations, echo=FALSE, warning=FALSE}
