...
In the original experiment, the two objects on day 1 were presented on two separate monitors and gaze was coded by manually coding video recordings. In our replication analysis, we analyzed eye movement data using ROIs. In this section we explore an alternative coding of the eye movement data by coding simply left half vs. right half of the screen. 
...
eyetracking.data <- eyetracking.data %>%
  ...
  mutate(in.left.roi = x < midpoint.x) %>%
  mutate(in.right.roi = x > midpoint.x)%>%
...
When looking scores are coded as left vs. right half of the screen, we do find that participants looked more at the novel object. The mean looking score was `r mean(t.test.data1$M.looking.score)` (*SD* = `r sd(t.test.data1$M.looking.score)`). This was significantly greater than 0.5, *t*(`r t.test.day1$parameter`) = `r t.test.day1$statistic`, *p* = `r t.test.day1$p.value`.
